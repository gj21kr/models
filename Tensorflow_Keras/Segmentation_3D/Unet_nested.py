...
from tensorflow.keras.layers import MaxPooling3D, Dropout, Conv3D, Conv3DTranspose, BatchNormalization, LeakyReLU, concatenate, Input
from tensorflow.keras.models import Model

def block_1(input_layer, nb_filter):
    conv1 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(input_layer)
    conv1 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(conv1)
    pool1 = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), )(conv1)
    return conv1, pool1

def block_2(input_layer, nb_filter):
    conv2 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(input_layer)
    conv2 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(conv2)
    pool2 = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), )(conv2)
    return conv2, pool2

def block_concat(input_layer, concat_layer, nb_filter, bn_axis = -1):
    up1_2 = Conv3DTranspose(nb_filter, (2, 2, 2), strides=(2, 2, 2), padding='same')(input_layer) 
    conv1_2 = concatenate([up1_2, concat_layer], axis=bn_axis)
    conv1_2 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(conv1_2)
    conv1_2 = Conv3D(nb_filter, (3, 3, 3), activation='relu', padding='same')(conv1_2) 
    return conv1_2
...
def unetpp(num_class=1, input_shape=(256,256,1), deep_supervision=4, nb_filter = [32,64,128,256,512]):
    ...
    # branch
    conv5_1 = Conv3D(nb_filter[4], (3, 3, 3), activation='relu', padding='same')(pool4)
    conv5_1 = Conv3D(nb_filter[4], (3, 3, 3), activation='relu', padding='same')(conv5_1)
    ...
    if deep_supervision==4:
        ...
        return Model(input=img_input, output=[output/4])  