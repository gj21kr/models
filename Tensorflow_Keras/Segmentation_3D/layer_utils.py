...
def encode_layer(X, channel, pool_size, pool, kernel_size='auto', 
    ...
    if pool == 'max':
        ...
        X = Conv3D(channel, kernel_size, strides=pool_size, 
...
def attention_gate(X, g, channel,  
    ...
    theta_att = Conv3D(channel, 1, use_bias=True, name='{}_theta_x'.format(name))(X)
    
    # mapping the gate tensor
    phi_g = Conv3D(channel, 1, use_bias=True, name='{}_phi_g'.format(name))(g)
    ...
    psi_f = Conv3D(1, 1, use_bias=True, name='{}_psi_f'.format(name))(f)
    # ------------------------------ #
...
def Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, dilation_rate=1, activation='ReLU', batch_norm=False, name='sep_conv'):
    ...
    for i in range(stack_num):
        X = DepthwiseConv3D(kernel_size, dilation_rate=dilation_rate, padding='same', 
        ...
        X = Conv3D(channel, (1, 1), padding='same', use_bias=bias_flag, name='{}_{}_pointwise'.format(name, i))(X)
...
def ASPP_conv(X, channel, activation='ReLU', batch_norm=True, name='aspp'):
    ...
    * dilation rates are fixed to `[6, 9, 12]`.
    ...
    b4 = GlobalAveragePooling3D(name='{}_avepool_b4'.format(name))(X)
    
    b4 = expand_dims(expand_dims(b4, 1), 1, name='{}_expdim_b4'.format(name))
    
    b4 = Conv3D(channel, 1, padding='same', use_bias=bias_flag, name='{}_conv_b4'.format(name))(b4)
    ...
    b0 = Conv3D(channel, (1, 1), padding='same', use_bias=bias_flag, name='{}_conv_b0'.format(name))(X)
