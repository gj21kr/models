...
# from tensorflow.keras.layers import DepthwiseConv3D
...
def SepConv_BN(x, filters, prefix, stride=1, kernel_size=3, rate=1, depth_activation=False, epsilon=1e-3):
    ...
    x = DepthwiseConv3D((kernel_size, kernel_size, kernel_size), 
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, skip_connection, rate=1):
    ...
    # Depthwise
    x = DepthwiseConv3D(kernel_size=3, strides=stride, activation=None,
...
def decoder(feature, skip1, img_input, classes, backbone, activation, callnum=1):
    ...
    if backbone == 'xception':
        ...
        size_now = tf.keras.backend.int_shape(x)
        factor = [int(tf.math.round(skip_size[i]/size_now[i])) for i in range(1,4)]
        x = tf.keras.backend.resize_volumes(x, depth_factor = factor[0], 
                                            height_factor = factor[1], 
                                            width_factor = factor[2], 
...
#         x = Lambda(lambda xx: tf.image.resize(xx, skip_size[1:3]))(x)
    ...
    size_now = tf.keras.backend.int_shape(x)
    factor = [int(tf.math.round(size_before3[i]/size_now[i])) for i in range(1,4)]
    x = tf.keras.backend.resize_volumes(x, depth_factor = factor[0], 
                                        height_factor = factor[1], 
                                        width_factor = factor[2], 
                                        data_format='channels_last')
...
def Deeplabv3(img_shape=(512, 512, 1), num_classes=1, 
    ...
    b4 = Lambda(lambda x: K.expand_dims(x, 1))(b4)
    b4 = Lambda(lambda x: K.expand_dims(x, 1))(b4)
    b4 = Conv3D(256, (1, 1, 1), padding='same',
                use_bias=False, name='image_pooling')(b4)
    b4 = BatchNormalization(name='image_pooling_BN', epsilon=1e-5)(b4)
    b4 = Activation(tf.nn.relu)(b4)
    # upsample. have to use compat because of the option align_corners
    size_before = tf.keras.backend.int_shape(x)
    size_now = tf.keras.backend.int_shape(b4)
    factor = [int(tf.math.round(size_before[i]/size_now[i])) for i in range(1,4)]
    b4 = tf.keras.backend.resize_volumes(b4, depth_factor = factor[0], 
                                        height_factor = factor[1], 
                                        width_factor = factor[2], 
                                        data_format='channels_last')
#     b4 = Lambda(lambda x: tf.image.resize(x, size_before[1:3]))(b4)
    # simple 1x1
    b0 = Conv3D(256, (1, 1, 1), padding='same', use_bias=False, name='aspp0')(x)
