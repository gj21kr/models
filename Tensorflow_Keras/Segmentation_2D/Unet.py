...
def unet(img_shape, n_shapes, depth = 4, features = 32, mode_norm='batch', data_format='channels_last'):
    ...
    conv6 = Conv2D(n_shapes, (1, 1), padding='same', data_format=data_format, activation=activation)(x)
...
def unet_edge(img_shape, n_shapes, n_edges, depth = 4, features = 32, mode_norm='batch', data_format='channels_last'):
    ...
    activation = return_activation(n_shapes)    
    activation_e = return_activation(n_edges)
    ...
    conv6 = Conv2D(n_shapes, (1, 1), padding='same', data_format=data_format, activation=activation)(x1)
...
def unet_sv(img_shape, n_shapes, depth = 4, features = 32, mode_norm='batch', data_format='channels_last'):
    ...
    for i in reversed(range(depth)):
        ...
        outs.append(Conv2D(n_shapes, (3,3), activation=activation, padding='same', name='out_{}'.format(i))(conv_out))
        up_size = up_size // 2
